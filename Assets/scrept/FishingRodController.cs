...
public class FishingRodController : MonoBehaviour
{
    ...
    [SerializeField] private AudioClip lineSnapSound; // ** NEW SOUND **
    ...
    private void ChangeState(FishingState newState)
    {
        ...
        switch (CurrentState)
        {
            ...
            case FishingState.FishCaught_WaitingToReel: hookedFish?.DisableAI(true); if(bobberInstance != null) bobberInstance.GetComponent<Rigidbody>().isKinematic = true; ShowNotification("Reel it in! (Hold E)", 0); break;
            case FishingState.ReelingInEmpty: StartCoroutine(ReelInEmptyBobberCoroutine()); break;
            case FishingState.LineSnapped: ShowNotification("Line Snapped!", 2f); mainAudioSource?.PlayOneShot(lineSnapSound); hookedFish?.ResetFishState(); hookedFish = null; cooldownTimer = cooldownDuration; break;
    ...
    private IEnumerator NotificationCoroutine(string message, float duration, bool animate) { if(notificationText != null) { notificationText.text = message; notificationText.gameObject.SetActive(true); if(animate) yield return AnimateSliderIn(notificationText.transform); } if (duration > 0) { yield return new WaitForSeconds(duration); if(notificationText != null) notificationText.gameObject.SetActive(false); } }
