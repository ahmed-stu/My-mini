...
{
    public enum FishingState { Idle, ChargingCast, Casting, WaitingForBite, FightingFish, FishCaught_WaitingToReel, ReelingInFish, ReelingInEmpty, LineSnapped, Cooldown }
    ...
    // ** NEW UI LABELS **
    [SerializeField] private TextMeshProUGUI fishHealthLabel;
    ...
    [SerializeField] private Color commonCatchColor = Color.white;
    [SerializeField] private Color rareCatchColor = new Color(0.2f, 0.6f, 1f); // Blue
    [SerializeField] private Color epicCatchColor = new Color(0.7f, 0.3f, 1f); // Purple
    [SerializeField] private Color legendaryCatchColor = new Color(1f, 0.8f, 0f); // Gold
    ...
    private float initialFishHealth;
    ...
    private void Awake()
    {
        ...
        fishHealthLabel?.gameObject.SetActive(false);
        tensionLabel?.gameObject.SetActive(false);
    }
    ...
    private void Update()
    {
        switch (CurrentState)
        {
            ...
            case FishingState.FishCaught_WaitingToReel: Update_FishCaught_WaitingToReel(); break;
            case FishingState.ReelingInFish: Update_ReelingInFish(); break;
    ...
    private void ChangeState(FishingState newState)
    {
        ...
        if (CurrentState == FishingState.FightingFish || CurrentState == FishingState.FishCaught_WaitingToReel) { playerAnimator?.SetBool("IsFighting", false); tensionAudioSource?.Stop(); StartCoroutine(AnimateSliderOut(fishHealthSlider?.transform)); StartCoroutine(AnimateSliderOut(tensionSlider?.transform)); StartCoroutine(AnimateSliderOut(fishHealthLabel?.transform)); StartCoroutine(AnimateSliderOut(tensionLabel?.transform)); notificationText?.gameObject.SetActive(false); }
        ...
        switch (CurrentState)
        {
            ...
            case FishingState.FightingFish: currentFishHealth = initialFishHealth; currentTension = 20f; splashTimer = 0.5f; if (fishHealthSlider != null) { fishHealthSlider.gameObject.SetActive(true); fishHealthSlider.maxValue = initialFishHealth; StartCoroutine(AnimateSliderIn(fishHealthSlider.transform)); if(fishHealthLabel != null) StartCoroutine(AnimateSliderIn(fishHealthLabel.transform)); } if (tensionSlider != null) { tensionSlider.gameObject.SetActive(true); tensionSlider.maxValue = maxTension; StartCoroutine(AnimateSliderIn(tensionSlider.transform)); if(tensionLabel != null) StartCoroutine(AnimateSliderIn(tensionLabel.transform)); } if (tensionAudioSource != null && lineTensionSound != null) { tensionAudioSource.clip = lineTensionSound; tensionAudioSource.Play(); } UpdateFightUI(); playerAnimator?.SetBool("IsFighting", true); break;
            case FishingState.FishCaught_WaitingToReel: hookedFish?.DisableAI(true); if(bobberInstance != null) bobberInstance.GetRigidbody().isKinematic = true; ShowNotification("Reel it in! (Hold E)", 0, Color.white); break;
            case FishingState.ReelingInEmpty: StartCoroutine(ReelInEmptyBobberCoroutine()); break;
            case FishingState.ReelingInFish: notificationText?.gameObject.SetActive(false); break;
            case FishingState.LineSnapped: ShowNotification("Line Snapped!", 2f, Color.red); mainAudioSource?.PlayOneShot(lineSnapSound); hookedFish?.ResetFishState(); hookedFish = null; cooldownTimer = cooldownDuration; break;
    ...
    private void Update_FightingFish() { if (bobberInstance != null && hookedFish != null) { bobberInstance.transform.position = Vector3.Lerp(bobberInstance.transform.position, hookedFish.transform.position, Time.deltaTime * bobberFollowSpeed); } currentTension += fishStruggleTension * Time.deltaTime; if (IsReelingInputHeld()) { currentTension += reelTensionIncrease * Time.deltaTime; currentFishHealth -= reelPower * Time.deltaTime; } else { currentTension -= tensionReleaseRate * Time.deltaTime; } currentTension = Mathf.Clamp(currentTension, 0, maxTension); currentFishHealth = Mathf.Clamp(currentFishHealth, 0, initialFishHealth); UpdateFightUI(); splashTimer -= Time.deltaTime; if (splashTimer <= 0f && currentTension > maxTension * 0.6f) { if(bobberInstance != null && bobberSplashEffect != null) Instantiate(bobberSplashEffect, bobberInstance.transform.position, Quaternion.identity); splashTimer = Random.Range(0.8f, 2.0f); } if (currentTension >= maxTension) ChangeState(FishingState.LineSnapped); else if (currentFishHealth <= 0f) ChangeState(FishingState.FishCaught_WaitingToReel); }
    private void Update_FishCaught_WaitingToReel() { if (IsReelingInputHeld()) ChangeState(FishingState.ReelingInFish); }
    private void Update_ReelingInFish() { if (catchZoneTransform == null || bobberInstance == null || hookedFish == null) { ChangeState(FishingState.Cooldown); return; } Transform bobberTransform = bobberInstance.transform; Transform fishTransform = hookedFish.transform; if (IsReelingInputHeld()) { bobberTransform.position = Vector3.MoveTowards(bobberTransform.position, catchZoneTransform.position, fishReelSpeed * Time.deltaTime); fishTransform.position = bobberTransform.position; } if (Vector3.Distance(bobberTransform.position, catchZoneTransform.position) < reelInFinishDistance) { FishData caughtFishData = hookedFish.GetFishData(); if (caughtFishData != null) { fishCaughtCount += caughtFishData.pointsValue; Color rarityColor = GetColorForRarity(caughtFishData.fishRarity); ShowNotification($"You Caught a {caughtFishData.fishName}!", 2f, rarityColor, true); } else { fishCaughtCount++; ShowNotification("You Caught a Fish!", 2f, Color.white, true); } UpdateScoreUI(); if (mainAudioSource != null && fishCaughtSound != null) mainAudioSource.PlayOneShot(fishCaughtSound); Destroy(hookedFish.gameObject); hookedFish = null; ChangeState(FishingState.Cooldown); } }
    ...
    public void StartFishFight(FishAI fish) { if (CurrentState == FishingState.WaitingForBite) { hookedFish = fish; FishData data = hookedFish.GetFishData(); if (data != null) initialFishHealth = data.health; ChangeState(FishingState.FightingFish); } }
    ...
    private IEnumerator ReelInEmptyBobberCoroutine() { if (bobberInstance == null || catchZoneTransform == null) { ChangeState(FishingState.Idle); yield break; } Transform bobberTransform = bobberInstance.transform; Rigidbody bobberRb = bobberInstance.GetRigidbody(); if (bobberRb != null) bobberRb.isKinematic = true; while (bobberTransform != null && Vector3.Distance(bobberTransform.position, catchZoneTransform.position) > reelInFinishDistance) { bobberTransform.position = Vector3.MoveTowards(bobberTransform.position, catchZoneTransform.position, reelInSpeed * Time.deltaTime); yield return null; } ChangeState(FishingState.Idle); }
    public bool IsReelingMinigameActive() => CurrentState == FishingState.FightingFish;
    
    private void UpdateFightUI() 
    { 
        float tensionRatio = currentTension / maxTension; 
        if (fishHealthSlider != null) 
        { 
            fishHealthSlider.value = currentFishHealth; 
            if (healthFillImage != null)
            {
                Color healthColor = healthColorGradient.Evaluate(currentFishHealth / initialFishHealth);
                healthFillImage.color = healthColor;
                if(fishHealthLabel != null) fishHealthLabel.color = healthColor;
            }
        } 
        if (tensionSlider != null && tensionFillImage != null) 
        { 
            tensionSlider.value = currentTension; 
            Color tensionColor;
            if (tensionRatio < 0.5f) tensionColor = safeColor; 
            else if (tensionRatio < 0.85f) tensionColor = warningColor; 
            else tensionColor = criticalColor;
            tensionFillImage.color = tensionColor;
            if(tensionLabel != null) tensionLabel.color = tensionColor;
        } 
        if (tensionAudioSource != null) { tensionAudioSource.pitch = Mathf.Lerp(1.0f, 2.5f, tensionRatio); tensionAudioSource.volume = Mathf.Clamp01(tensionRatio * 1.2f); } 
    }
    private void UpdateScoreUI() { if (scoreText != null) scoreText.text = $"Score: {fishCaughtCount}"; }
    private void ShowNotification(string message, float duration, Color color, bool animate = false) { if (gameObject.activeInHierarchy) StartCoroutine(NotificationCoroutine(message, duration, color, animate)); }
    private IEnumerator NotificationCoroutine(string message, float duration, Color color, bool animate) { if(notificationText != null) { notificationText.color = color; notificationText.text = message; notificationText.gameObject.SetActive(true); if(animate) yield return AnimateSliderIn(notificationText.transform); } if (duration > 0) { yield return new WaitForSeconds(duration); if(notificationText != null) notificationText.gameObject.SetActive(false); } }
    ...
    private Color GetColorForRarity(FishData.Rarity rarity)
    {
        switch (rarity)
        {
            case FishData.Rarity.Common:
                return commonCatchColor;
            case FishData.Rarity.Rare:
                return rareCatchColor;
            case FishData.Rarity.Epic:
                return epicCatchColor;
            // ** BUG FIX: Corrected the enum type from FishState to FishData **
            case FishData.Rarity.Legendary:
                return legendaryCatchColor;
            default:
                return Color.white;
        }
    }
